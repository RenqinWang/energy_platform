...
| `silver_point_meta_dim` | `point_code` | `point_code`、`point_name`、`station_id`、`system_type`、`equipment_id`、`theme`、`unit` | 点位字典维表 |
...
1. **状态型字段**(运行开关、启停标志):优先采用前值保持,若前值距当前超过 1 小时则显式置为 `unknown`。
2. **连续型字段**(温度、压力、流量):
   - 连续缺失 ≤ 5 个采样周期(假设采样间隔 1 分钟即 5 分钟)为短时缺失,采用最近邻或时间窗口线性插值。
...
4. **价格字段**:若当天价格缺失,回退到最近一个有效价格,记录 `quality_flag = 'price_fallback'`。
