...
# -*- coding: utf-8 -*-
"""
从Kafka采集数据到Bronze层（修正版）
"""

from pyspark.sql import SparkSession
from pyspark.sql.functions import col, from_json, current_timestamp, to_date, lit, regexp_replace
...
def create_spark_session():
    ...
    spark = (
        ...
        .appName("Load_Kafka_To_Bronze_V2")
        .master("local[*]")
...
def main():
    ...
    try:
        # 使用subscribePattern匹配所有sensor_开头的topic
        df_kafka = spark.read \
            .format("kafka") \
        ...
        # 展开数据并清理sensor_id（移除sensor_前缀）
        df_final = df_parsed.select(
            col("data.timestamp").alias("timestamp"),
            col("data.value").alias("value"),
            regexp_replace(col("topic"), "sensor_", "").alias("sensor_id"),
